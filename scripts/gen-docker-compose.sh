...
export REDIS_PORT POSTGRES_PORT POSTGRES_DB POSTGRES_USER POSTGRES_PASSWORD \
  API_PORT QUEUE_KEY LOG_LEVEL CONCURRENCY \
...
envsubst '$REDIS_PORT $POSTGRES_PORT $POSTGRES_DB $POSTGRES_USER $POSTGRES_PASSWORD $API_PORT $QUEUE_KEY $LOG_LEVEL $CONCURRENCY $REDIS_HEALTHCHECK_INTERVAL $REDIS_HEALTHCHECK_TIMEOUT $REDIS_HEALTHCHECK_RETRIES $POSTGRES_HEALTHCHECK_INTERVAL $POSTGRES_HEALTHCHECK_TIMEOUT $POSTGRES_HEALTHCHECK_RETRIES' \
